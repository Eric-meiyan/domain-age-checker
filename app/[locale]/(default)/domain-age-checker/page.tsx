...
export default function DomainAgeCheckerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const domainData = domainList.map(fullDomain => {
        const parts = fullDomain.split('.');
        if (parts.length < 2) {
          throw new Error(`${t('error_invalid_domain')}: ${fullDomain}`);
        }
        return {
      ...
      const keywords = domainData.map(d => d.keyword);
      const tlds = [...new Set(domainData.map(d => d.tld))];
  ...
  const formatDomainAge = (days?: number) => {
    ...
    const parts = [];
    if (years > 0) parts.push(`${years}${locale === 'zh' ? '年' : ' years'}`);
    if (months > 0) parts.push(`${months}${locale === 'zh' ? '月' : ' months'}`);
    if (finalDays > 0) parts.push(`${finalDays}${locale === 'zh' ? '天' : ' days'}`);

    return parts.length > 0 ? parts.join(' ') : (locale === 'zh' ? '不到1天' : 'Less than 1 day');
  };
